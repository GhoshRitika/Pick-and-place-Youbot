"""Run 'python3 code/overshoot.py' from the Ghosh_Ritika_capstone directory. It will automatically save
csv files, log file and plots in the Results/Overshoot directory.
"""
...
def main():
    logging.basicConfig(filename ='Results/Overshoot/runscript.log', level = logging.INFO)
    ...
    Tse_init = np.array([[ 0, 0, 1,   0], 
                         ...
                         [-1, 0, 0, 0.5],
                         [ 0, 0, 0,   1]])
    # the cube's initial configuration (x, y, theta) = (1, 0, 0)
    Tsc_init = np.array([[1, 0, 0,     1],
                         [0, 1, 0,     0],
                         [0, 0, 1, 0.025],
                         [0, 0, 0,     1]])
    # the cube's desired final configuration (x, y, theta) = (0, -1, -pi/2)
    ...
    #Generates the overshoot csv one
    logging.info('Generating overshoot case animation csv file')
    np.savetxt("Results/Overshoot/overshoot.csv", config_list, delimiter = ",")
    logging.info('Writing error plot data')
    np.savetxt("Results/Overshoot/X_error_overshoot.csv", Xerr_plot, delimiter = ",")
    ...
    plt.ylabel('Error')
    plt.legend(loc='best')
    plt.savefig(f'Results/Overshoot/X_error OVERSHOOT Kp= {Kp_gain} and Ki= {Ki_gain}.png')
    plt.show()
    ...
    fig.savefig(f'Results/Overshoot/X_error OVERSHOOT display Kp= {Kp_gain} and Ki= {Ki_gain}.png')
    plt.show()
    logging.info('Done')
