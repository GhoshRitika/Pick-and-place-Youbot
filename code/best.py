"""Run 'python3 code/best.py' from the Ghosh_Ritika_capstone directory. It will automatically save
csv files, log file and plots in the Results/Best directory.
"""
...
def main():
    logging.basicConfig(filename ='Results/Best/runscript.log', level = logging.INFO)
    ...
    #initial position and orientation error
    ...
    logging.info('Generating best case animation csv file')
    np.savetxt("Results/Best/best.csv", config_list, delimiter = ",")
    logging.info('Writing error plot data')
    np.savetxt("Results/Best/X_error_BEST.csv", Xerr_plot, delimiter = ",")
    ...
    plt.ylabel('Error')
    plt.legend(loc='best')
    plt.savefig(f'Results/Best/X_error BEST Kp= {Kp_gain} and Ki= {Ki_gain}.png')
    plt.show()
    ...
    fig.savefig(f'Results/Best/X_error BEST display Kp= {Kp_gain} and Ki= {Ki_gain}.png')
    plt.show()
    logging.info('Done')
